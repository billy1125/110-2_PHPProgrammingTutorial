...
function Account_Check(string $_account, string $_password)
{
    ...
    try {
        if ($link === false) {
            ...
            $sql = "SELECT id, account, password, name, admin FROM users WHERE account = ? AND password = ? AND del = 0";
...
function Add_Member(string $_account, string $_password, string $_name, string $_admin)
{
    ...
    return $AddResult; // 跑完讀檔後，最終整個函式會丟出去 $AddResult 變數
}
...
function Delete_Member(string $_id)
{
    ...
    try {
        if ($link === false) {
            ...
            $sql = "UPDATE users SET del = 1 WHERE id = ?";
